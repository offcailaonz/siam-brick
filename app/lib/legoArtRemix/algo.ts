...
export function alignPixelsToStudMap(inputPixels, studMap, colorDistanceFunction) {
...
export function getAverageQuantizationError(pixels1, pixels2, colorDistanceFunction) {
    let totalError = 0;
...
export function findReplacement(pixelRGB, remainingStudMap, colorDistanceFunction) {
    const possibleReplacements = Object.keys(remainingStudMap);
    let replacement = possibleReplacements[0];
    possibleReplacements.forEach((possibleReplacement) => {
        if (
...
export function getDepthSubPixelMatrix(pixelArray, totalWidth, horizontalOffset, verticalOffset, width, height) {
    const result = [];
    for (var i = 0; i < pixelArray.length / 4; i++) {
...
export function getSubPixelMatrix(pixelMatrix, horizontalOffset, verticalOffset, width, height) {
    const result = [];
...
export function resizeImageArrayWithAdaptivePooling(input2DArray, outputWidth, outputHeight, subArrayPoolingFunction) {
    const result = [];
...
export function drawDepthPlatesCountForContext(usedDepthParts, scalingFactor, ctx, horizontalOffset, verticalOffset) {
...
export function generateDepthInstructionPage(perDepthLevelMatrices, scalingFactor, canvas, plateNumber) {
    const ctx = canvas.getContext("2d");
...
export const DEPTH_FILLER_PARTS = Object.keys(PLATE_DIMENSIONS_TO_PART_ID).map((part) =>
...
export function getVariablePixelWantedListXML(pixelColorMatrix, variablePixelPieceDimensions, pixelType) {
